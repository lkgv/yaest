...
fn revert_pos_arg(
    ...
    if let Some(path) = path {
        ...
        if ns.target == Target::Solana {
            ns.diagnostics.push(Diagnostic::error(
                *loc,
                format!("SEMA_STATEMENT_ERROR: revert with custom errors not supported on {}", ns.target),
            ));
        }
        */
...
fn try_catch(
    ...
    if ns.target == Target::Solana {
        diagnostics.push(Diagnostic::error(
            *loc,
            "SEMA_STATEMENT_ERROR: The try-catch statement is not supported on Solana. Please, go to \
