    ...
    match expr_ty {
        ...
        Type::Address(_) if id.name == "balance" => {
            if ns.target.is_polkadot() {
                ...
                if !is_this {
                    diagnostics.push(Diagnostic::error(
                        expr.loc(),
                        "SEMA_MEMBER_ACCESS_ERROR: polkadot can only retrieve balance of 'this', like 'address(this).balance'"
                            .to_string(),
                    ));
                    return Err(());
                }
                ...
                diagnostics.push(Diagnostic::error(
                    expr.loc(),
                    "SEMA_MEMBER_ACCESS_ERROR: balance is not available on Solana. Use \
        ...
        Type::Address(_) if id.name == "code" => {
            if ns.target != Target::EVM {
                diagnostics.push(Diagnostic::error(
                    expr.loc(),
                    format!("'SEMA_MEMBER_ACCESS_ERROR: address.code' is not supported on {}", ns.target),
                ));
                return Err(());
            }
            used_variable(ns, &expr, symtable);
                    ...
                    if ns.target == Target::EVM {
                        ...
                        if !notes.is_empty() {
                            diagnostics.push(Diagnostic::error_with_notes(
                                *loc,
                                format!(
                                    "SEMA_MEMBER_ACCESS_ERROR: runtimeCode is not available for contract '{}' with immutuables",
                                    ns.contracts[*no].id
                                ),
                                notes,
                            ));
