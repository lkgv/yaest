    ...
    if n.starts_with("0x") && !n.chars().any(|c| c == '_') && n.len() == 42 {
        ...
        if ns.target == Target::EVM {
            ...
            diagnostics.push(Diagnostic::error(
                *loc,
                format!(
                    "SEMA_EXPRESSION_LITERALS_ERROR: ethereum address literal '{}' not supported on target {}",
                    n, ns.target
                ),
            ));
            return Err(());
...
pub(super) fn address_literal(
    ...
    if ns.target.is_polkadot() {
        ...
        diagnostics.push(Diagnostic::error(
            *loc,
            format!("SEMA_EXPRESSION_LITERALS_ERROR: address literal {} not supported on {}", address, ns.target),
        ));
        Err(())
    }
}
