    ...
    if let Some(namespace) = &namespace {
        ...
        if namespace.name == "super" {
            if let Some(cur_contract_no) = context.contract_no {
                ...
                diagnostics.push(Diagnostic::error(
                    *loc,
                    "SEMA_FUNCTION_CALL_ERROR: super not available outside contracts".to_string(),
                ));
                return Err(());
            }
        }
    ...
    match var_ty.deref_any() {
        ...
        Type::Array(..) if func.name == "push" || func.name == "pop" => {
            diagnostics.push(Diagnostic::error(
                func.loc,
                format!(
                    "SEMA_FUNCTION_CALL_ERROR: method {}() is not available for fixed length arrays",
                    func.name
                ),
            ));
            return Err(());
            ...
            if func.name == "transfer" || func.name == "send" {
                if ns.target == Target::Solana {
                    diagnostics.push(Diagnostic::error(
                        *loc,
                        format!(
                            "SEMA_FUNCTION_CALL_ERROR: method '{}' not available on Solana. Use the lamports \
                ...
                if !is_payable {
                    diagnostics.push(Diagnostic::error(
                        *loc,
                        format!(
                            "SEMA_FUNCTION_CALL_ERROR: method '{}' available on type 'address payable' not 'address'",
                            func.name,
                        ),
                    ));
